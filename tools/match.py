        ...
        try:
            ...
            while not board.is_game_over(claim_draw=True) and board.ply() < MAX_PLIES:
                ...
                try:
                    ...
                    raise RuntimeError(f"illegal move {move} from {engine.path}: {exc}") from exc

            if board.is_checkmate():
                result, reason = ("0-1", "checkmate") if board.turn == chess.WHITE else ("1-0", "checkmate")
            elif board.is_stalemate() or board.is_insufficient_material() or board.is_seventyfive_moves() or board.is_fivefold_repetition():
                result, reason = "1/2-1/2", "draw-rule"
            else:
            ...
            failures.append(f"GAME {gi+1}: {type(exc).__name__}: {exc}")
            print(f"FAIL GAME {gi+1}: {type(exc).__name__}: {exc}", flush=True)
        finally:
            if a: a.close()
            if c: c.close()
